...
class SmdImporter(bpy.types.Operator, Logger):
	...
	lock_position : BoolProperty(name="Lock position", description="", default=False)
	...
	def applyFrames(self,keyframes,num_frames, fps = None):
		...
		if smd.jobType == ANIM:
			...
			if 'ActLib' in dir(bpy.types):
				...
				bpy.context.scene.frame_end = (num_frames - 1) * 4
			...
			def ApplyRecursive(bone, lock_position):
						...
						if keyframe.rot:
							if smd.rotMode == 'XYZ':
								for i in range(3):
									...
									curvesRot[i].keyframe_points[-1].co = [keyframe.frame * 4, bone.rotation_euler[i]]
							else:
								for i in range(4):
									curvesRot[i].keyframe_points.add(1)
									curvesRot[i].keyframe_points[-1].co = [keyframe.frame * 4, bone.rotation_quaternion[i]]
				# Recurse
				for child in bone.children:
					ApplyRecursive(child, False)
			
			# Start keying
			for bone in smd.a.pose.bones:			
				if not bone.parent:
					ApplyRecursive(bone, self.lock_position)
	...
	def readSMD(self, filepath, upAxis, rotMode, newscene = False, smd_type = None, target_layer = 0):
		...
		try:
			smd.file = file = open(filepath, 'r', errors='replace')
